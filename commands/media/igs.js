 ...
 * igs - Converter mídia do Instagram em figurinha (com padding, mantém proporção)
 * igsc - Converter mídia do Instagram em figurinha quadrada recortada
 */
...
async function convertBufferToStickerWebp(inputBuffer, isAnimated, cropSquare) {
  ...
  if (inputBuffer.length > MAX_FILE_SIZE) {
    throw new Error(`Arquivo muito grande: ${(inputBuffer.length / 1024 / 1024).toFixed(2)}MB (máx: ${MAX_FILE_SIZE / 1024 / 1024}MB)`);
  }
  ...
  try {
    ...
    const json = {
      ...
      'sticker-pack-name': config.packname || 'Feito por',
      'emojis': ['📸']
    };
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      if (!isValidContentType(contentType)) {
        throw new Error(`Tipo de conteúdo inválido: ${contentType} (esperado image/* ou video/*)`);
      }

      if (isHtmlResponse(buffer)) {
        throw new Error('Resposta é HTML (bloqueado/login necessário)');
      }
      ...
      if (attempt === maxRetries - 1) {
        try {
          ...
          if (isHtmlResponse(buffer)) {
            throw new Error('Resposta do stream é HTML (bloqueado/login necessário)');
          }

          return buffer;
        } catch (streamError) {
          throw new Error(`Falha ao baixar mídia após ${maxRetries} tentativas: ${lastError?.message || lastError}`);
        }
      }
...
async function igsCommand(sock, msg, args, extra, crop = false) {
  try {
    ...
    if (items.length === 0) {
      return extra.reply('❌ Nenhuma mídia encontrada no link fornecido.');
    }
...
module.exports = {
  name: 'igs',
  aliases: ['igsticker'],
  description: 'Converter post/reel do Instagram em figurinha (mantém proporção com padding)',
  usage: '.igs <URL do Instagram>',
  category: 'media',
